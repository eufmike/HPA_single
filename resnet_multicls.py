...
from func.trainer_mlgpu import load_train_objs, prepare_dataloader, Trainer
...
def dltrain(rank, world_size, save_every=None, max_epochs=None, batch_size=None):

    # create log
    prj_dir = Path(
        "/dlab/ldrive/CBT/USLJ-DSDE_DATA-I10008/shihch3/projects/HPA_single_data"
    )
    ...
    datadir = Path(
        "/dlab/ldrive/CBT/USLJ-DSDE_DATA-I10008/BenchmarkDatasets/hpa-single-cell-image-classification"
    )
...
def main(args):
    ...
    mp.spawn(dltrain, args=([world_size]), nprocs=world_size)
    return
...
if __name__ == "__main__":
    ...
    reqdarg = parser.add_argument(
        "-g", dest="gpu_device", default=0, type=int, nargs="+", help="gpu device"
    )
